...
sheets_list = ['AccumulatedAnnualDemand','CapacityFactor','CapitalCost','EmissionActivityRatio','EmissionsPenalty','FixedCost', 'InputActivityRatio','OperationalLife','OutputActivityRatio','ResidualCapacity','SpecifiedAnnualDemand','SpecifiedDemandProfile','TotalAnnualMaxCapacity','TotalTechnologyAnnualActivityLo','TotalTechnologyAnnualActivityUp','VariableCost','YearSplit']
#sheets_list = [
 #'AvailabilityFactor',
 #'CapacityFactor',
...
for i in range(len(sheets_list)):
    ...
    df_param.to_csv( './1_Parameters/'+ sce + '/' + csv_name +'.csv', index = None, header=True)
    
    
    
file_table1 = './0_Ref/STRUCTURE_OSEMOSYS_GUA_LAND.xls'
file_txt = './0_Ref/data_land.txt'

pre_file_name = './1_Parameters/' + sce + '/'
...
g= open( './2_Model/' + sce + '/data_land' + '.txt',"w+")
g.write( '###############\n#    Sets     #\n###############\n#\n' )
...
#%%%   
        if len(this_param_keys) == 5:
            ...
            for k in range(int(len(techs)/61)):
                index = int(61*k)
...
#g.write('param TotalTechnologyAnnualActivityLowerLimit default 0 :=\n;\n')
