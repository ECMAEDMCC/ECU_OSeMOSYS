...
# CONFIGURACIÓN DE ESCENARIOS PARA EMISIONES - INTEGRACIÓN NDC
try:
    base_configuration_EAR = pd.read_excel('B1_Scenario_Config.xlsx', sheet_name='EmissionsActivityRatio')
    print("✓ Archivo B1_Scenario_Config.xlsx leído correctamente")
    print(f"✓ Encontradas {len(base_configuration_EAR)} configuraciones de emisiones")
except FileNotFoundError:
    print("⚠ Advertencia: No se encontró el archivo B1_Scenario_Config.xlsx")
    print("⚠ Se usarán valores por defecto para las emisiones")
    base_configuration_EAR = None
except Exception as e:
    print(f"⚠ Error leyendo B1_Scenario_Config.xlsx: {e}")
    print("⚠ Se usarán valores por defecto para las emisiones")
    base_configuration_EAR = None
#
    ...
    for n in range( len( df_index_list ) ):
        ...
        if s != 0: # s == 0 does not need to change in time
            ...
            if this_projection_mode == 'Flat':
                for y in range( len( time_range_vector ) ):
                    this_df_new.loc[ n, time_range_vector[y] ] = round( this_df.loc[ n, 2010 ], 4 )
...
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
# SECCIÓN DE EMISIONES - MEJORADA CON ENFOQUE NDC
print("Procesando emisiones con configuraciones de escenario...")
...
# Procesamiento base de GHGs
print("=== DEPURACIÓN: PROCESANDO EMISIONES GHGs ===")

# DEPURACIÓN: Mostrar configuraciones disponibles
if base_configuration_EAR is not None:
    print(f"DEBUG: Configuraciones de EmissionActivityRatio disponibles:")
    print(base_configuration_EAR)
    print(f"DEBUG: Columnas disponibles: {base_configuration_EAR.columns.tolist()}")
else:
    print("DEBUG: NO hay configuraciones de EmissionActivityRatio disponibles")

for e in range(len(these_emissions)):
    this_emission = these_emissions[e]
    this_tech = these_e_techs[e]
    
    print(f"DEBUG: Procesando {this_tech} - {this_emission}")
    
    for y in range(len(time_range_vector)):
        # Verificar si hay configuraciones específicas de escenario
        emission_value = these_e_values[e]  # Valor por defecto
        
        # Si existe configuración de escenario, aplicar modificaciones
        if base_configuration_EAR is not None:
            try:
                scenario_config = base_configuration_EAR[
                    (base_configuration_EAR['Set'] == this_tech) & 
                    (base_configuration_EAR['Parameter'] == 'EmissionActivityRatio')
                ]
                
                print(f"DEBUG: Buscando configuración para {this_tech}-{this_emission}")
                print(f"DEBUG: Configuraciones encontradas: {len(scenario_config)}")
                
                if not scenario_config.empty:
                    print(f"DEBUG: ¡ENCONTRADA configuración para {this_tech}-{this_emission}!")
                    print(scenario_config[['Scenario', 'Parameter', 'Set']])
                    
                    # Aplicar factor de modificación si existe
                    if 'Factor' in scenario_config.columns:
                        original_value = these_e_values[e]
                        factor = scenario_config['Factor'].iloc[0]
                        emission_value = original_value * factor
                        print(f"DEBUG: APLICANDO FACTOR - {this_tech}-{this_emission}: {original_value} × {factor} = {emission_value}")
                    elif time_range_vector[y] in scenario_config.columns:
                        original_value = these_e_values[e]
                        emission_value = scenario_config[time_range_vector[y]].iloc[0]
                        print(f"DEBUG: APLICANDO VALOR ESPECÍFICO - {this_tech}-{this_emission} año {time_range_vector[y]}: {original_value} → {emission_value}")
                else:
                    print(f"DEBUG: NO encontrada configuración para {this_tech}-{this_emission}")
                    
            except Exception as e_error:
                print(f"DEBUG ERROR aplicando configuración: {e_error}")
        ...
        if y == 0:  # Solo imprimir para el primer año para no saturar
            print(f"DEBUG: Fila creada - {this_tech}-{this_emission}-{time_range_vector[y]}: {emission_value}")

# Procesamiento de Externalities
df_EmissionPenalty = pd.DataFrame( columns = Wide_Param_Header )
these_emissions_ext = Emissions_ext_df['External Cost'].tolist()
these_e_techs_ext = Emissions_ext_df['Tech'].tolist()
these_e_values_ext = Emissions_ext_df['EmissionActivityRatio'].tolist()
these_e_penalty = Emissions_ext_df['EmissionsPenalty'].tolist()
accumulated_emission_penalty_data = []
emission_unique_set = set()

for e in range(len(these_emissions_ext)):
    this_emission = these_emissions_ext[e]
    this_tech = these_e_techs_ext[e]
    
    for y in range(len(time_range_vector)):
        # EmissionActivityRatio para externalities
        emission_value = these_e_values_ext[e]
        
        # Aplicar configuraciones de escenario si existen
        if base_configuration_EAR is not None:
            try:
                scenario_config = base_configuration_EAR[
                    (base_configuration_EAR['Set'] == this_tech) & 
                ...
                if not scenario_config.empty:
                    if 'Factor' in scenario_config.columns:
                        emission_value = these_e_values_ext[e] * scenario_config['Factor'].iloc[0]
                    elif time_range_vector[y] in scenario_config.columns:
                        emission_value = scenario_config[time_range_vector[y]].iloc[0]
            except:
                pass
        ...
        # EmissionsPenalty (solo una vez por emisión-año)
...
# Actualizar diccionario principal
overall_param_df_dict['EmissionActivityRatio'] = df_Emissions
overall_param_df_dict['EmissionsPenalty'] = df_EmissionPenalty

print(f"✓ Procesadas {len(df_Emissions)} filas de EmissionActivityRatio")
print(f"✓ Procesadas {len(df_EmissionPenalty)} filas de EmissionsPenalty")
#
...
Demand_df_new[2010] = Demand_df_new[2010].astype(float)
...
for d in range( len( list_dicts ) ):
    ...
    df_to_print.to_csv( './A2_Output_Params/DDP70/' + list_dicts[d] + '.csv', index=False, header=True) #TODO: this changes with scenarios folders, fix
    df_to_print = df_to_print.replace( { 'Scenario':{ "DDP70":"PLANDEACCION"} } )
    df_to_print.to_csv( './A2_Output_Params/PLANDEACCION/' + list_dicts[d] + '.csv', index=False, header=True)
#
end_2 = time.time()   
