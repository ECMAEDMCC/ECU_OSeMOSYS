# -*- coding: utf-8 -*-
"""
@author: Luis Victor-Gallardo // 2023
"""
import errno
import scipy
import pandas as pd
...
def main_executer(n1, packaged_useful_elements, scenario_list_print, start1):
    ...
    #if data_file.__contains__('TENDENCIAL'):  # CAMBIAR EL NOMBRE SEGUN EL ESCENARIO QUE SE QUIERA GENERAR EL .lp
    #        str2 = "glpsol -m OSeMOSYS_Model.txt -d " + str(data_file)  +  " --wlp energia.lp "
    #else:
    #    str2 = "glpsol -m OSeMOSYS_Model.txt -d " + str(data_file) 
    ...
    #
    end_1c = time.time()   
    time_elapsed_1c = -start1 + end_1c
    print(str(time_elapsed_1c) + ' seconds /', str(time_elapsed_1c/60) + ' minutes | Phase C')
...
def data_processor(case, unpackaged_useful_elements):
    ...
    df_fuel_2_code_fuel_list = unpackaged_useful_elements[8]
    df_fuel_2_code_plain_english = unpackaged_useful_elements[9]
    df_tech_2_code_fuel_list = unpackaged_useful_elements[10]
    df_tech_2_code_plain_english = unpackaged_useful_elements[11]
    #
    # 1 - Always call the structure of the model:
    sheet_sets_structure = unpackaged_useful_elements[-3]
    sheet_params_structure = unpackaged_useful_elements[-2]
    sheet_vars_structure = unpackaged_useful_elements[-1]
    #-------------------------------------------#
    ...
    for var in range(len(vars_as_appear)):
        ...
        for k in range(len(this_var_dict[this_variable])):
                ...
                if this_tech in list(Fleet_Groups_inv.keys()):
                    ...
                    if 'RAI' not in group_tech:
                        ...
                        driven_distance_final = \
                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(time_range_vector[-1])])
                        ...
                        if this_variable == 'NewCapacity':
                            ...
                            if int(this_year) >= 2024 and driven_distance_2024 != driven_distance_final:
                                ...
                                if int(this_year) > 2010+Reference_op_life[this_strategy][this_tech]:  # works with a timeframe by 2050; longer timeframes may need more lifetime cycle adjustments
            ...
            #
            if this_combination[2] in ['Accidents', 'Health', 'Congestion', 'CO2_scc', 'CO2_fugitivas_scc'] and this_variable == 'AnnualTechnologyEmissionPenaltyByEmission':
                ref_index = combination_list.index(this_combination)
                this_data_row = deepcopy(data_row_list[ref_index]) # this must be updated in a further position of the list
                #
                ...
                if this_tech in list(Fleet_Groups_inv.keys()):
                    ...
                    if 'RAI' not in group_tech:
                        ...
                        if int(this_year) >= 2024 and driven_distance_2024 != driven_distance_final:  # greater than the base year
                            ...
                            if int(this_year) > 2010+Reference_op_life[this_strategy][this_tech]:
                                subtract_index_accum_old = \
                                    int(Reference_op_life[this_strategy][this_tech])
                                if int(this_year) - subtract_index_accum_old <= 2024:
                                    driven_distance_17 = \
...
def function_C_mathprog(scen, stable_scenarios, unpackaged_useful_elements):
    ...
    #
    # Briefly open up the system coding to use when processing for visualization:
    df_fuel_2_code_fuel_list = unpackaged_useful_elements[10]
    df_fuel_2_code_plain_english = unpackaged_useful_elements[11]
    df_tech_2_code_fuel_list = unpackaged_useful_elements[12]
    df_tech_2_code_plain_english = unpackaged_useful_elements[13]
    #
...
if __name__ == '__main__':
    ...
    '''
    print('Check possible inputs')

    # Open the "ignore" list:
    df_other_techs = \
        pd.read_excel('./A1_Outputs/A-O_Parametrization.xlsx', sheet_name='Other_Techs')
    list_other_techs = list(set(df_other_techs['Tech'].tolist()))
    ...
    *Abbreviations:*
    ...
    Fleet_Groups_OR =           pickle.load(open('./A1_Outputs/A-O_Fleet_Groups_OR.pickle',         "rb"))
    Fleet_Groups_techs_2_dem =  pickle.load(open('./A1_Outputs/A-O_Fleet_Groups_T2D.pickle',        "rb"))
    Fleet_Groups_techs_list = \
        ['TRNMIC', 'TRNBUS', 'TRNTAX', 'TRNPASRAI', 'TRNMOT', 'TRNSED',
         'TRNSUV', 'TRNCAM', 'TRNFREHEA', 'TRNCATTRU', 'TRNFRERAI',
         'TRNFREMED', 'TRNFRELIG']
    #
    Fleet_Groups_inv = {}
    for k in range(len(list(Fleet_Groups.keys()))):
    ...
    for n in range(len(base_configuration_modeshift.index)):
        ...
        if str(base_configuration_modeshift.loc[n ,'Logistic']) == 'YES' and str(base_configuration_modeshift.loc[n ,'Linear']) == 'NO':
            modeshift_params[base_configuration_modeshift_unique_scenario_list[-1]][base_configuration_modeshift_group_set_list[-1]].update({'Values':[],'Type':'Logistic'}) # WE MUST REMEMBER THE ORDER OF APPENDING THE PARAMETERS OF LOGISTIC BEHAVIOR: R2023, R2050, L, C, M
            ...
            #
            modeshift_params[base_configuration_modeshift_unique_scenario_list[-1]][base_configuration_modeshift_group_set_list[-1]]['Values'].append(float(base_configuration_modeshift.loc[n ,'R2023']))
        ...
        #
            ...
            modeshift_params[base_configuration_modeshift_unique_scenario_list[-1]][base_configuration_modeshift_group_set_list[-1]]['Values'].append(base_configuration_modeshift.loc[n ,'v_2060'])
            modeshift_params[base_configuration_modeshift_unique_scenario_list[-1]][base_configuration_modeshift_group_set_list[-1]]['Values'].append(base_configuration_modeshift.loc[n ,'v_2070'])
            #
        #
    ...
    for n in range(len(base_configuration_adoption.index)):
        ...
        if str(base_configuration_adoption.loc[n ,'Logistic']) == 'YES' and str(base_configuration_adoption.loc[n ,'Linear']) == 'NO':
            adoption_params[base_configuration_adoption_unique_scenario_list[-1]][base_configuration_adoption_group_set_list[-1]].update({'Values':[], 'Type':'Logistic', 'Restriction_Type':str(base_configuration_adoption.loc[n ,'Restriction_Type']) }) # WE MUST REMEMBER THE ORDER OF APPENDING THE PARAMETERS OF LOGISTIC BEHAVIOR: R2023, R2050, L, C, M
            #
            adoption_params[base_configuration_adoption_unique_scenario_list[-1]][base_configuration_adoption_group_set_list[-1]]['Values'].append(float(base_configuration_adoption.loc[n ,'R2023']))
        ...
        #
            ...
            adoption_params[base_configuration_adoption_unique_scenario_list[-1]][base_configuration_adoption_group_set_list[-1]]['Values'].append(base_configuration_adoption.loc[n ,'v_2060'])
            adoption_params[base_configuration_adoption_unique_scenario_list[-1]][base_configuration_adoption_group_set_list[-1]]['Values'].append(base_configuration_adoption.loc[n ,'v_2070'])
            #
        #
            ...
            adoption_params[base_configuration_adoption_unique_scenario_list[-1]][base_configuration_adoption_group_set_list[-1]]['Values'].append(float(base_configuration_adoption.loc[n ,'y_ini']))
            colval_vector = ['e' + str(j) for j in range(2023,2070+1)]
    ...
    transport_group_sets = [i for i in transport_group_sets if 'RAI' not in i]
    ...
    # print('check reference driven distance')
    # sys.exit()
    '''''
    ################################# PART 2 #################################
    '''''
    ...
    for s in range(len(scenario_list)):
        ...
        passpub_range_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['SpecifiedAnnualDemand'][this_set_type_initial]) if x == str('DEMTRNPASPUB')]
        passpriv_range_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['SpecifiedAnnualDemand'][this_set_type_initial]) if x == str('DEMTRNPASPRI')]
        ...
        #
            ...
            # print('review if modeshift is good')
            # sys.exit()
            ...
            for a_set in range(len(applicable_sets_demand)): # FLAG: in the future we must consider non-logistic behaviors
                ...
                if modeshift_params[scenario_list[s]][this_set]['Type'] == 'Logistic':
                    #
                    R2023 = modeshift_params[scenario_list[s]][this_set]['Values'][0] # This is not used / remove in future versions.
                    ...
                    #
                    shift_years = [n for n in 
                                   range(Initial_Year_of_Uncertainty+1,
                                         time_range_vector[-1]+1)]
                ...
                #
                    ...
                    v_2060 = modeshift_params[scenario_list[s]][this_set]['Values'][4]
                    v_2070 = modeshift_params[scenario_list[s]][this_set]['Values'][5]
                    #
                    ...
                    for y in range(len(time_range_vector)):
                        ...
                        if v_2050 != 'interp' and time_range_vector[y] == 2050:
                            ...
                            not_known_e = False
                        if v_2060 != 'interp' and time_range_vector[y] == 2060:
                            xp_coord_known.append(y)
                            yp_coord_known.append(v_2060)
                            not_known_e = False
                        if v_2070 != 'interp' and time_range_vector[y] == 2070:
                            xp_coord_known.append(y)
                            yp_coord_known.append(v_2070)
                            not_known_e = False
                        if not_known_e == True:
                    ...
                    #  
                #
                if 'DEMTRNPASPUB' in this_set:
                    new_value_list_PUB = []
                elif 'DEMTRN_NOMOT' in this_set:
                    new_value_list_NOMOT = []
                #
                for n in range(len(time_range_vector)):               
                    if 'DEMTRNPASPUB' in this_set: # we must subtract the rail activity and then add it again above
            ...
            for a_set in range(len(applicable_sets_tech)): # REMEMBER THIS IS A CHANGE FUNCTION
                ...
                if this_set not in Fleet_Groups_techs_list:
                ...
                if modeshift_params[scenario_list[s]][this_set]['Type'] == 'Linear':
                    ...
                    if this_set == 'TRNPASRAIELE':
                        # stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['value'][this_set_range_indices[0]:this_set_range_indices[-1]+1] = deepcopy([1.0001*i for i in new_value_range_set_group_rounded])
                        # stable_scenarios[scenario_list[s]]['TotalTechnologyAnnualActivityLowerLimit']['value'][this_set_range_indices_lm[0]:this_set_range_indices_lm[-1]+1] = deepcopy([0.9999*i for i in new_value_range_set_group_rounded])
                        ...
                        # this_set = 'TRNPASRAIELE'
                        # this_set_range_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['t']) if x == str(this_set)]
                        # this_set_range_indices_lm = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['TotalTechnologyAnnualActivityLowerLimit']['t']) if x == str(this_set)]
                        # x1 = stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['value'][this_set_range_indices[0]:this_set_range_indices[-1]+1]
                        # x2 = stable_scenarios[scenario_list[s]]['TotalTechnologyAnnualActivityLowerLimit']['value'][this_set_range_indices_lm[0]:this_set_range_indices_lm[-1]+1]
                        # sys.exit()

                        # print('happened?')
                        # sys.exit()

                    # Store the value of freight rail (electric) below
                    adjust_fre_rai = True
                    if this_set == 'TRNFRERAIELE' and adjust_fre_rai is True:
                        ...
                        for this_parameter in ['TotalAnnualMaxCapacity', 'TotalTechnologyAnnualActivityLowerLimit']:
                            # We must adjust the capacity of the complementary set of *TRNFRERAI*
                            this_set_complement_dict = {'TRNFRERAI':'TRNFREHEA'}
                            this_set_complement = this_set_complement_dict[this_set_group]
                            ...
                            if this_parameter == 'TotalAnnualMaxCapacity':
                                security_multiplier_factor = 1.0001
                            elif this_parameter == 'TotalTechnologyAnnualActivityLowerLimit':
                                security_multiplier_factor = 0.9999
                        ...
                        # if this_set == 'TRNFRERAIELE':
                        #    print('check 2')
                        #    sys.exit()

                #
                """
                if modeshift_params[scenario_list[s]][this_set]['Type'] == 'Linear' and this_set == 'TRNPASRAIELE':  # *ALERT* add the elimination of rail as a possible tool
                    this_set_range_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['t']) if x == str(this_set)]
                ...
                if modeshift_params[scenario_list[s]][this_set]['Type'] == 'Linear' and this_set == 'TRNPASRAIELE':  # *ALERT* add the elimination of rail as a possible tool
                    ...
                    #
                    # Set zero for DDP (total capacity)
                    ...
                    #
                    # Set zero for DDP (costs for passenger rail)
                    cost_sets_eliminate = ['EXTRATECH']
                    ...
                    #
                    # Re-establish BAU for DDP (group capacity)
        ...
        for capvar in range(len(capacity_variables)):
            ...
            for g in range(len(group_tech_PRIVATE)):
                ...
                #
                else: # if scenario_list[s] in ['DDP','NDP_A','OP15C']: # we must apply the same distribution of transport in pkm as in the BAU, considering the new demands. The variable to modify is the capacity.
                    #
            ...
            for g in range(len(group_tech_PUBLIC)):
                ...
                if scenario_list[s] == 'BAU':
                    ...
                    #
                    if 'RAI' not in group_tech_PUBLIC[g]: # we need to find the shares of non-rail modes:
                        cap_indices_rail = [i for i, x in enumerate(stable_scenarios[scenario_list[s]][capacity_variables[capvar]][this_set_type_initial]) if x == str('TRNPASRAI')]
                ...
                #
                    ...
                    if adjust_with_rail == False:
                        ...
                        stable_scenarios[scenario_list[s]][capacity_variables[capvar]]['value'][cap_indices[0]:cap_indices[-1]+1] = deepcopy(new_cap_values_rounded)
                    #
                    elif 'RAI' not in group_tech_PUBLIC[g]:
                        or_values = deepcopy(stable_scenarios[scenario_list[s]]['OutputActivityRatio']['value'][or_indices[0]:or_indices[-1]+1])
                        or_values = [float(or_values[j]) for j in range(len(or_values))]
                        #
                        # here we quickly call the associated demand
                        ...
                        for year in range(13, 2070 - 2010 + 1):  # start from 1 (2019) up to 2070 - 2010 + 1 (2070)
                            # Check if the rate is decreasing
                            is_decreasing = new_cap_values_rounded[year] < new_cap_values_rounded[year - 1]
                            # Store the result in the dictionary
                            is_decreasing_dict[2010 + year] = is_decreasing
            ...
            for g in range(len(group_tech_FREIGHT_HEA)):
                ...
                if scenario_list[s] == 'BAU':
                    ...
                    #
                    if 'RAI' not in group_tech_FREIGHT_HEA[g]: # we need to find the shares of non-rail modes:
                        cap_indices_rail = [i for i, x in enumerate(stable_scenarios[scenario_list[s]][capacity_variables[capvar]][this_set_type_initial]) if x == str('TRNFRERAI')]
                        cap_values_rail = deepcopy(stable_scenarios[scenario_list[s]][capacity_variables[capvar]]['value'][cap_indices_rail[0]:cap_indices_rail[-1]+1])
                        cap_values_rail = [float(cap_values_rail[j]) for j in range(len(cap_values_rail))]
                        demand_values_norail = [demand_values[j] - cap_values_rail[j] for j in range(len(demand_values))]
                        #
                ...
                #
                    ...
                    if adjust_with_rail == False:
                        ...
                        stable_scenarios[scenario_list[s]][capacity_variables[capvar]]['value'][cap_indices[0]:cap_indices[-1]+1] = deepcopy(new_cap_values_rounded)
                    #
                    elif 'RAI' not in group_tech_FREIGHT_HEA[g]:
                        or_values = deepcopy(stable_scenarios[scenario_list[s]]['OutputActivityRatio']['value'][or_indices[0]:or_indices[-1]+1])
                        or_values = [float(or_values[j]) for j in range(len(or_values))]
                        #
                        # here we quickly call the associated demand
        ...
        if scenario_list[s] in list(set(base_configuration_transport_elasticity['Scenario'].tolist())):            
            ...
            if elasticity_Params_method_all == 'Exact':
                Sets_Involved = deepcopy(Fleet_Groups_techs_list)
                ...
                for par in range(len(params_to_adjust)):
                    for a_set in range(len(Sets_Involved)):
                        ...
                        if 'PUB' in str(Fleet_Groups_techs_2_dem[Sets_Involved[a_set]]):
                            ...
                            train_pass_capacity_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['t']) if x == str('TRNPASRAI')]
                            ...
                            if Fleet_Groups_techs_2_dem[Sets_Involved[a_set]] == 'DEMTRNPASPUB' and scenario_list[s] == 'DDP':
        ...
        df_other_techs = \
            pd.read_excel('./A1_Outputs/A-O_Parametrization.xlsx', sheet_name='Other_Techs')
        list_other_techs = list(set(df_other_techs['Tech'].tolist()))
        ignore_techs = deepcopy(list_other_techs)
        rewrite_techs_maxcap = \
            [e for e in unique_max_cap_techs if
             'TRN' in e and
             'IN' not in e and
             e not in list(Fleet_Groups.keys()) and
             e not in ignore_techs]
        ...
        for t in range(len(rewrite_techs_maxcap)):
            ...
            if 0 in freeze_group_value_list:
                ...
                value_list = [1.0001*group_value_list[i]*freeze_value_list[i]/freeze_group_value_list[i] for i in range(len(group_value_list))]
        ...
        for t in range(len(rewrite_techs_lowlim)):
            ...
            if 0 in freeze_group_value_list:
                ...
                value_list = [0.9999*group_value_list[i]*freeze_value_list[i]/freeze_group_value_list[i] for i in range(len(group_value_list))]
            value_list_rounded = [round(elem, 4) for elem in value_list]
        ...
        if scenario_list[s] in list(set(list(adoption_params.keys()))):
            ...
            for a_set in range(len(applicable_sets)):
                ...
                if this_type == 'Linear':
                    ...
                    v_2060 = adoption_params[scenario_list[s]][this_set]['Values'][4]
                    v_2070 = adoption_params[scenario_list[s]][this_set]['Values'][5]
                    #
                    known_years_ini = [e for e in range(2010, int(y_ini)+1)]
                    known_years = known_years_ini + \
                    ...
                    #2060
                    try:
                        float_v2060 = float(v_2060)
                    except Exception:    
                        float_v2060 = 'interp'
                    #2070
                    try:
                        float_v2070 = float(v_2070)
                    except Exception:
                        float_v2070 = 'interp'
                    #
                    ...
                    else:  # If there is no prior restriction, we start from zero.
                        known_value_ini = []
                        for y in range(len(time_range_vector)):
                            if time_range_vector[y] <= y_ini:
                                known_value_ini.append(0)

                    known_values = known_value_ini + \
                        [float_v2030, float_v2040, float_v2050, float_v2060,
                         float_v2070]
                    #
                    x_coord_tofill, xp_coord_known, yp_coord_known = [], [], []
                    for y in range(len(time_range_vector)):
                ...
                if this_type == 'Logistic':
                    R2023 = adoption_params[scenario_list[s]][this_set]['Values'][0] # This is not used / remove in future versions.
                ...
                if this_type == 'Exact':
                    ...
                    if len(this_set_range_indices) != 0:
                        ...
                        known_value_ini = [new_adoption_shift[0]]*14
                        
                    #for i in range(len(new_adoption_shift)):
                    #    if 'fill' == new_adoption_shift[i]:
                    #        new_adoption_shift[i] = known_value_ini[-1]
                    
                    adoption_shift = known_value_ini + new_adoption_shift
                    
                    #if this_set == 'TRNFREHEAELE':
                    #    print('review this hypothesis please')
                    #    sys.exit()
                ...
                for cp in range(len(cap_vars)):
                    ...
                    if cap_vars[cp] in ['TotalAnnualMaxCapacity', 'TotalTechnologyAnnualActivityUpperLimit']:
                        security_multiplier_factor = 1.0001 # 1.0001
                    elif cap_vars[cp] == 'TotalTechnologyAnnualActivityLowerLimit':
                        security_multiplier_factor = 0.9999 # 0.9999
                    #
                    ...
                    if len(this_set_range_indices) != 0:
                        ...
                        # print('happens??', this_set, this_sector, scenario_list[s], cap_vars[cp])
                        for y in range(len(time_range_vector)):
                            stable_scenarios[scenario_list[s]][cap_vars[cp]]['r'].append('ECU')
            ...
            if scenario_list[s] != 'BAU':
                ...
                # print('check the sets that are reset')    
                # sys.exit()
        ...
        if scenario_list[s] != 'BAU': # we proceed to adjust first the parameters and corresponding sets affected by distance, then the demands.
            for p in range(len(parameters_involved)):
                ...
                for a_set in range(len(transport_group_sets)):
                    ...
                    if ((this_demand_set == 'DEMTRNPASPRI' and dem_adj_private is True) or
                        (this_demand_set == 'DEMTRNPASPUB' and dem_adj_public is True) or
                        (this_demand_set == 'DEMTRNFREHEA' and dem_adj_heafre is True) or
                        (this_demand_set == 'DEMTRNFREMED' and dem_adj_medfre is True) or
                        (this_demand_set == 'DEMTRNFRELIG' and dem_adj_ligfre is True)):
                        ...
                        if Fleet_Groups_techs_2_dem[this_group_set] == 'DEMTRNPASPUB' and scenario_list[s] == 'DDP':
                            subtract_list = [float(train_pass_capacity_values[j]) for j in range(len(train_pass_capacity_values))]
                        else:
                            subtract_list = [0 for j in range(len(train_pass_capacity_values))]
                        ...
                        #                                       
                        if this_demand_set == 'DEMTRNPASPRI':
                            demand_indices = [i for i, x in enumerate(stable_scenarios[scenario_list[s]]['SpecifiedAnnualDemand']['f']) if x == str('DEMTRN_NOMOT')]
                            demand_list = deepcopy(stable_scenarios[scenario_list[s]]['SpecifiedAnnualDemand']['value'][demand_indices[0]:demand_indices[-1]+1])
                            #
                            new_value_list = []
                            for n in range(len(demand_list)):
                            ...
                            #
                        if this_demand_set == 'DEMTRNPASPUB':
                            dem_adj_public = False
                            #
                        if this_demand_set == 'DEMTRNFREHEA':
                            dem_adj_heafre = False
                            #
                        if this_demand_set == 'DEMTRNFREMED':
                            dem_adj_medfre = False
                            #
                        if this_demand_set == 'DEMTRNFRELIG':
                            dem_adj_ligfre = False

                    # group-tech capacity adjustments are done second:
                    if ('TRN' in this_group_set and this_parameter in ['TotalAnnualMaxCapacity', 'TotalTechnologyAnnualActivityLowerLimit']): # THIS CONDITION WILL ALWAYS BE TRUE
                        #
                        ...
                        if len(this_set_range_indices) != 0:
                            ...
                            if this_group_set == 'TRNFRERAI' and scenario_list[s] != 'BAU':  # here we need to adjust the capacity of freight rail for consistency
                                tsri_frerail = [i for i, x in enumerate(stable_scenarios[scenario_list[s]][this_parameter][this_set_type_initial]) if x == str('TRNFRERAI')]
        ...
        for g in range(len(transport_group_sets)): # this actually avoids the maxcap issue
            ...
            value_list = [1.0001*float(value_list[j]) for j in range(len(value_list))]
            #
        ...
        unique_max_cap_techs = list(set(stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['t'])) + ['FSTCHRGER']
        unique_low_lim_techs = list(set(stable_scenarios[scenario_list[s]]['TotalTechnologyAnnualActivityLowerLimit']['t']))
        #
        for g in range(len(transport_group_sets)): # this actually avoids the maxcap issue
            this_group_set = transport_group_sets[g]
            ...
            for t in range(len(this_tech_set_list)):
                ...
                if this_tech in unique_residual_cap_techs and this_tech not in unique_max_cap_techs:
                    ...
                    #
                    for y in range(len(time_range_vector)):
                        stable_scenarios[scenario_list[s]]['TotalAnnualMaxCapacity']['r'].append('ECU')
    ...
    for sb in range(len(scenario_list_based)):
        if scenario_list_based[sb] != 'ref':
            ...
            if scenario_list_based[sb] != 'BAU':
                ...
                Blend_Shares.update({ this_scenario_name:Blend_Shares['DDP'] })
                ...
                for p in range(len(param_list)):
                    ...
                    if electrical_Params_built_in == 'NO':
                        ...
                        if 'Write' in electrical_Params_methods:
                            ...
                            for y in range(len(time_range_vector)):
                                stable_scenarios[this_scenario_name][param_list[p]]['r'].append('ECU')
    ...
    # ###########################################################################
    # ###########################################################################
    # print(scenario_list[s], 'demand debug -1')
    # dict_BAU = stable_scenarios['BAU']
    # #
    # group_techs = list(Fleet_Groups_techs_2_dem.keys())
    # #
    # dem_sets = []
    # dem_sets_max_vals = {}
    # time_range_vector = [y for y in range(2010, 2070+1)]
    ...
    """
    # E1_ETA
    this_param_indices_t = [i for i, x in enumerate(stable_scenarios['DDP']['InputActivityRatio']['t']) if x == str('BLEND_GSL')]
    this_param_indices_f = [i for i, x in enumerate(stable_scenarios['DDP']['InputActivityRatio']['f']) if x == str('E1_PURGSL')]
    # this_param_indices_f = [i for i, x in enumerate(stable_scenarios['DDP']['InputActivityRatio']['f']) if x == str('E1_ETA')]
    this_param_indices_raw = list(set(this_param_indices_t) & set(this_param_indices_f))
    check_year_list_raw = [stable_scenarios['DDP']['InputActivityRatio']['y'][i] for i in this_param_indices_raw]
    this_param_indices = list(set(this_param_indices_t) & set(this_param_indices_f))
    this_param_indices.sort()
    check_year_list = [stable_scenarios['DDP']['InputActivityRatio']['y'][i] for i in this_param_indices]
    blend_value_list = deepcopy(stable_scenarios['DDP']['InputActivityRatio']['value'][this_param_indices[0]:this_param_indices[-1]+1])
    """

    """
    this_param_indices_t = [i for i, x in enumerate(stable_scenarios['DDP']['EmissionActivityRatio']['t']) if x == str('T4_GSLPUB')]
    this_param_indices_e = [i for i, x in enumerate(stable_scenarios['DDP']['EmissionActivityRatio']['e']) if x == str('CO2')]
    # this_param_indices_t = [i for i, x in enumerate(stable_scenarios['BAU']['EmissionActivityRatio']['t']) if x == str('T4_GSLPUB')]
    # this_param_indices_e = [i for i, x in enumerate(stable_scenarios['BAU']['EmissionActivityRatio']['e']) if x == str('CO2')]
    this_param_indices = list(set(this_param_indices_t) & set(this_param_indices_e))
    this_param_indices.sort()
    emiss_value_list = deepcopy(stable_scenarios['DDP']['EmissionActivityRatio']['value'][this_param_indices[0]:this_param_indices[-1]+1])
    """
    ...
    if is_this_last_update == True:
        ...
        end_1a = time.time()   
        time_elapsed_1a = -start1 + end_1a
        print(str(time_elapsed_1a) + ' seconds /', str(time_elapsed_1a/60) + ' minutes | Phase A')

    #########################################################################################
    global scenario_list_print
    scenario_list_print = deepcopy(scenario_list) # ['BAU','OP15C','DDP']

    # print('stop before printing')
    # sys.exit()

    # Open files that are used internally in each function:
    ...
    if generator_or_executor == 'Generator' or generator_or_executor == 'Both':
        ...
        packaged_useful_elements = \
            [scenario_list_print, S_DICT_sets_structure,
             S_DICT_params_structure, list_param_default_value_params,
             list_param_default_value_value, print_adress,
             Reference_driven_distance, Fleet_Groups_inv, time_range_vector,
             Blend_Shares,
             df_fuel_2_code_fuel_list, df_fuel_2_code_plain_english,
             df_tech_2_code_fuel_list, df_tech_2_code_plain_english]
        #
        ...
        end_1b = time.time()   
        time_elapsed_1b = -start1 + end_1b
        print(str(time_elapsed_1b) + ' seconds /', str(time_elapsed_1b/60) + ' minutes | Phase B')
    ...
    if generator_or_executor == 'Executor' or generator_or_executor == 'Both':
        ...
        #
        packaged_useful_elements = \
            [Reference_driven_distance, Reference_occupancy_rate,
             Reference_op_life, Fleet_Groups_inv, time_range_vector,
             list_param_default_value_params, list_param_default_value_value,
             list_gdp_ref,
             df_fuel_2_code_fuel_list, df_fuel_2_code_plain_english,
             df_tech_2_code_fuel_list, df_tech_2_code_plain_english,
             sheet_sets_structure, sheet_params_structure,
             sheet_vars_structure]
        #
        set_first_list(scenario_list_print)
        ...
        for n in range(0,y_ceil):
            ...
            #    
            for n2 in range(n_ini , max_iter):
                p = mp.Process(target=main_executer, args=(n2, packaged_useful_elements, scenario_list_print, start1))
                processes.append(p)
                p.start()
            #
            for process in processes:
