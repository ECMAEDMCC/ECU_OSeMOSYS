...
Fuels_dems = [i for i in range(len(list_fuel_or_tech)) if 'DEMTRN' in list_fuel_or_tech[i]] 
Fuels_techs = [i for i in range(len(list_fuel_or_tech))
               if ('TRN' in list_fuel_or_tech[i] and 
                   'DEM' not in list_fuel_or_tech[i] and
                   'E5' not in list_fuel_or_tech[i])]
Fuels_techs_2_dems = {}
...
for m in range(len(list_demand_or_share)):
    ...
    if 'GDP' in list_projection_mode[m]:
        ...
        if 'joint' in list_projection_mode[m]:
            ...
            other_value_BY = Demand_df.loc[other_tech_index, 2010]
        this_value_BY = Demand_df.loc[m, 2010]
        #
    ...
    if 'Population coupling' in list_projection_mode[m]:
        this_tech = list_fuel_or_tech[m]
        this_net_value_BY = Demand_df.loc[m, 2010]
        demand_trajectory = [this_net_value_BY]
        for y in range(len(Projections_sheet['Year'].tolist())):
...
for g in range(len(groups_list)):
    ...
    if isinstance(oar, (float, int)):
        for d in range(this_group_tech_index):
            if 'DEMTRN' in Demand_df_new.loc[d, 'Fuel/Tech']:
